...
{
    int retVal = -1;
    if (cfg_get_prplmesh_param_int("daisy_chaining_disabled", &retVal) == RETURN_ERR) {
        return false;
    }
...
{
    std::string option = "daisy_chaining_disabled";
    std::string value  = std::to_string(((int)daisy_chaining_disabled));
...
{
    int retVal = -1;
    if (cfg_get_prplmesh_param_int("channel_select_task_enabled", &retVal) == RETURN_ERR) {
        return false;
    }
...
{
    int retVal = -1;
    if (cfg_get_prplmesh_param_int("dfs_task_enabled", &retVal) == RETURN_ERR) {
        return false;
    }
...
int cfg_get_backhaul_params(int *max_vaps, int *network_enabled, int *preferred_radio_band)
{
    ...
    if (preferred_radio_band) {
        ...
        } else if (preferred_bh_band.compare("S1G")) {
            *preferred_radio_band = BPL_RADIO_BAND_S1G;
#endif
