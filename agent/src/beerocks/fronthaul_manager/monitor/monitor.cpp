...
bool Monitor::update_sta_qos_ctrl_params()
{
    for (auto it = mon_db.sta_begin(); it != mon_db.sta_end(); ++it) {
        ...
        if (sta_node == nullptr) {
            LOG(ERROR) << "Invalid node pointer for STA = " << sta_mac;
            return false;
        }
...
void Monitor::handle_cmdu_vs_message(ieee1905_1::CmduMessageRx &cmdu_rx)
{
    ...
    case beerocks_message::ACTION_MONITOR_CLIENT_UNASSOCIATED_STA_LINK_METRIC_REQUEST: {
        LOG(DEBUG) << "received ACTION_MONITOR_CLIENT_UNASSOCIATED_STA_LINK_METRIC_REQUEST";
        auto request = beerocks_header->addClass<
    ...
    case Event::Unassociation_Stations_Stats: {
        ...
        for (auto &station_out : msg->un_stations_stats) {
            ...
            beerocks_message::sUnassociatedStationStats un_sta_stats = {
                station_out.mac_adress,      station_out.signal_strength, station_out.channel,
                station_out.operating_class, station_out.time_stamp,
            };
