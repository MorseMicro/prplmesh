    ...
    case beerocks_message::ACTION_BACKHAUL_RADIO_TEAR_DOWN_REQUEST: {
        ...
        if (db->device_conf.certification_mode) {
            ...
            for (const auto &radio_manager_element : m_radio_managers.get()) {
                ...
                for (uint8_t vap_idx = 0; vap_idx < eBeeRocksIfaceIds::IFACE_TOTAL_VAPS;
                     vap_idx++) {
                    ...
                    wifi_credentials->authentication_type_attr().data =
                        WSC::eWscAuth::WSC_AUTH_INVALID;
                                                  ...
                                                  std::shared_ptr<beerocks_header> beerocks_header)
{
    ...
    case beerocks_message::ACTION_APMANAGER_HOSTAP_SPATIAL_REUSE_REPORT_NOTIFICATION: {
        ...
        if (!notification) {
            LOG(ERROR)
                << "addClass ACTION_APMANAGER_HOSTAP_SPATIAL_REUSE_REPORT_NOTIFICATION failed";
            return false;
        }
        ...
        LOG(DEBUG) << "received ACTION_APMANAGER_HOSTAP_SPATIAL_REUSE_REPORT_NOTIFICATION from "
                   << sender_iface_name;
